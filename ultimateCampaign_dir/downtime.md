---
layout: default
title: ultimateCampaign_dir/downtime
---
# Downtime

[Downtime Activities](ultimateCampaign_dir/downtime_dir/downtimeActivities) [Managers](ultimateCampaign_dir/downtime_dir/managers) [Rooms and Teams](ultimateCampaign_dir/downtime_dir/roomsAndTeams) [Buildings and Organizations](ultimateCampaign_dir/downtime_dir/buildingsAndOrganizations) [Downtime Events](ultimateCampaign_dir/downtime_dir/downtimeEvents)
...
Even if you don't want to use the rules to earn extra gold or throw your weight around in town as a business owner, there are campaign and roleplaying benefits for using the downtime system. For example, if you build or buy a house, you have a comfortable, private place to rest between adventures. By adding a few more rooms, you can easily convert a house into a base of operations for your adventuring party; it would count as "very familiar" for the purpose of your _ [teleport](spells_dir/teleport#_teleport)_ spells, and if it includes an altar to your deity, you can use it as the destination for a _ [word of recall](spells_dir/wordOfRecall#_word-of-recall)_.
...
**Followers** : Followers are a type of Labor gained from the [Leadership](feats#_leadership) feat or other methods. Followers can be used like Labor, but aren't expended like capital because they are loyal to you and don't leave as soon as an activity is completed. For more information, see Using Followers on page 80.
...
**Gold Pieces** : Gold pieces (or gp) constitute the normal money your character has, such as from looting monsters or earning a living with [Craft](skills_dir/craft#_craft) or [Profession](skills_dir/profession#_profession) checks. Many downtime activities require you to spend gp.
...
**Skilled Work** : If you have ranks in a useful skill, you can spend 1 day working in a settlement to earn more capital than you would doing unskilled work. Note that this method includes both legal and illegal means of earning capital—for example, a day spent using [Sleight of Hand](skills_dir/sleightOfHand#_sleight-of-hand) to earn money could be a day spent performing as a street magician or a day spent pickpocketing.
...
If you are using this option to earn Goods, Influence, Labor, or Magic, the skill you're using must be suitable for earning the chosen type of capital; if the GM deems it is not, using that skill reduces the amount generated by half (minimum 1). For example, [Perform](skills_dir/perform#_perform) might earn you Influence as a musician, but it's not as useful for earning Labor. The GM should inform you of this before you attempt the skill check. In general, the appropriate skills for each type of capital are as follows.

_Goods_: [Appraise](skills_dir/appraise#_appraise), [Bluff](skills_dir/bluff#_bluff), [Craft](skills_dir/craft#_craft), [Diplomacy](skills_dir/diplomacy#_diplomacy), [Disable Device](skills_dir/disableDevice#_disable-device), [Handle Animal](skills_dir/handleAnimal#_handle-animal), [Intimidate](skills_dir/intimidate#_intimidate), [Knowledge](skills_dir/knowledge#_knowledge) (dungeoneering, engineering, geography, history, local, nature, nobility, religion), [Profession](skills_dir/profession#_profession), [Sleight of Hand](skills_dir/sleightOfHand#_sleight-of-hand), [Stealth](skills_dir/stealth#_stealth).

_Influence_: [Appraise](skills_dir/appraise#_appraise), [Bluff](skills_dir/bluff#_bluff), [Craft](skills_dir/craft#_craft), [Diplomacy](skills_dir/diplomacy#_diplomacy), [Handle Animal](skills_dir/handleAnimal#_handle-animal), [Heal](skills_dir/heal#_heal), [Intimidate](skills_dir/intimidate#_intimidate), [Knowledge](skills_dir/knowledge#_knowledge) (any), [Linguistics](skills_dir/linguistics#_linguistics), [Perform](skills_dir/perform#_perform), [Profession](skills_dir/profession#_profession), [Ride](skills_dir/ride#_ride).

_Labor_: [Bluff](skills_dir/bluff#_bluff), [Climb](skills_dir/climb#_climb), [Craft](skills_dir/craft#_craft), [Diplomacy](skills_dir/diplomacy#_diplomacy), [Handle Animal](skills_dir/handleAnimal#_handle-animal), [Intimidate](skills_dir/intimidate#_intimidate), [Knowledge](skills_dir/knowledge#_knowledge) (local), [Profession](skills_dir/profession#_profession), [Ride](skills_dir/ride#_ride), [Survival](skills_dir/survival#_survival), [Swim](skills_dir/swim#_swim).

_Magic_: [Appraise](skills_dir/appraise#_appraise), [Craft](skills_dir/craft#_craft), [Diplomacy](skills_dir/diplomacy#_diplomacy), [Heal](skills_dir/heal#_heal), [Knowledge](skills_dir/knowledge#_knowledge) (arcana, dungeoneering, nature, planes, religion), [Linguistics](skills_dir/linguistics#_linguistics), [Profession](skills_dir/profession#_profession), [Spellcraft](skills_dir/spellcraft#_spellcraft), [Use Magic Device](skills_dir/useMagicDevice#_use-magic-device).

The value of a particular skill for a given type of capital can vary from settlement to settlement. For example, in a frontier settlement with a tradition of serious hard work, a day of humorous performances using [Perform](skills_dir/perform#_perform) (comedy) might not earn you much capital, but inspirational public speeches about the city's heroes using [Knowledge](skills_dir/knowledge#_knowledge) (history) or [Perform](skills_dir/perform#_perform) (oratory) could. The GM should tell you this before you attempt the skill check, or allow you to assess the inhabitants' preferences with a successful DC 15 [Knowledge](skills_dir/knowledge#_knowledge) (local) or [Sense Motive](skills_dir/senseMotive#_sense-motive) check.
...
**Phase 2—Activity** : [Perform](skills_dir/perform#_perform) downtime activities, such as constructing a building, recruiting an organization, or retraining.
...
Because adventuring is dangerous work, if you're away from a settlement for 30 days or more, you risk losing control of your businesses there as employees begin to wonder whether you're dead. Upon your return, you must attempt a leadership check (1d20 + your [Leadership](feats#_leadership) score) against a DC equal to the number of days since you last had contact with that businesses — 10 (so if you've been gone for 30 days, the DC is 20). Having contact with the business requires visiting it personally, sending a qualified representative on your behalf (such as a cohort or manager; see page 88), or sending a formal letter or magical communication (such as _ [dream](spells_dir/dream#_dream)_, _ [sending](spells_dir/sending#_sending)_, or _ [whispering wind](spells_dir/whisperingWind#_whispering-wind)_); doing so resets your number of days away to 0.
